...
FIREBASE_PROJECT="${UNTETHERED_FIREBASE_PROJECT:-${OLAUNCHER_FIREBASE_PROJECT:-${FIREBASE_PROJECT:-olauncher-focus-sspen}}}"
FIREBASE_APP_ID="${UNTETHERED_FIREBASE_APP_ID:-${OLAUNCHER_FIREBASE_APP_ID:-${FIREBASE_APP_ID:-1:1702988366:android:6cb561b20112533c0dd11f}}}"
FIREBASE_GROUPS="${UNTETHERED_FIREBASE_GROUPS:-${OLAUNCHER_FIREBASE_GROUPS:-${FIREBASE_GROUPS:-}}}"
FIREBASE_TESTERS="${UNTETHERED_FIREBASE_TESTERS:-${OLAUNCHER_FIREBASE_TESTERS:-${FIREBASE_TESTERS:-}}}"
FIREBASE_RELEASE_NOTES="${UNTETHERED_FIREBASE_RELEASE_NOTES:-${OLAUNCHER_FIREBASE_RELEASE_NOTES:-${FIREBASE_RELEASE_NOTES:-}}}"
FIREBASE_RELEASE_NOTES_FILE="${UNTETHERED_FIREBASE_RELEASE_NOTES_FILE:-${OLAUNCHER_FIREBASE_RELEASE_NOTES_FILE:-${FIREBASE_RELEASE_NOTES_FILE:-}}}"
...
Legacy OLAUNCHER_FIREBASE_* variables are still accepted.

The debug APK package is app.untethered.debug, so the Firebase Android app must
...
if [[ -z "$FIREBASE_GROUPS" && -z "$FIREBASE_TESTERS" ]]; then
  FIREBASE_TESTERS="s.spencer.a@gmail.com"
fi
...
if [[ -n "$FIREBASE_RELEASE_NOTES_FILE" ]]; then
  ...
  ARGS+=(--release-notes "Untethered focus build $(git -C "$REPO_ROOT" rev-parse --short HEAD)")
fi
